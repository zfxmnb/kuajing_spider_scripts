window.temu_helper_v2_core = async (fetchInterceptor) => {
  ...
  let mallId =
    ...
    window.localStorage.getItem('agentseller-mall-info-id') ||
    window.localStorage.getItem('dxmManualCrawlMallId');
  try {
  ...
  try {
    getCacheItem = GM_getValue ?? localStorage.getItem;
  } catch (err) {}
  let setCacheItem = localStorage.setItem;
  try {
    setCacheItem = GM_setValue ?? localStorage.setItem;
  } catch (err) {}
  ...
  const findElementsByText = (text, parent = document) => {
    // 查找包含特定文本的元素节点
    const matchingNodes = document.evaluate(`.//*[contains(text(), "${text}") and not(self::script)]`, parent, null, XPathResult.ORDERED_NODE_ITERATOR_TYPE);
    const matchedNodes = [];
    let node;
    ...
    pageItems.forEach(({ productSkuSummaries, productId, productSkcId, productName }) => {
      productSkuSummaries?.forEach(({ productSkuId, supplierPrice, siteSupplierPrices, virtualStock, productSkuSpecList }) => {
        ...
        skus.push({
          ...
          sku: productSkuId,
          title: productName,
          price: numberFixed((siteSupplierPrices?.[0]?.supplierPrice ?? supplierPrice) / 100),
          stock: virtualStock,
          specList,
          skuNum: productSkuSummaries?.length || 1
        });
      });
    });
    return skus;
  ...
  const updateMmsBtgProductSalesStock = async (productId, skuId, skuStockChangeList) => {
    return await fetch('/darwin-mms/api/kiana/ghost/btg/sales/stock/updateMmsBtgProductSalesStock', {
      ...
      body: JSON.stringify({
        productId: productId,
        skuStockChangeList: skuStockChangeList?.map((props) => ({ productSkuId: skuId, currentShippingMode: 1, ...props })),
        skuTypeChangeList: [],
        isCheckVersion: true,
        source: 0
      })
      ...
      afterSales.push({
        ...
        orders: afterSalesItemVOList?.map(({ orderSn, orderStatus, productSkuId: sku, originalSpecName, originalGoodsName, goodsName } = {}) => {
          ...
          return {
            ...
            orderId: orderSn,
            sku,
            title: currentOrderDataMap[id]?.title || `${originalSpecName ? `【${originalSpecName}】 ` : ''}${goodsName || originalGoodsName}`,
  ...
  async function getOrderPackages(id) {
    return fetch('/mms/eagle/package/main_batch_query', {
      ...
      body: JSON.stringify({ logistics_create_time: [], parent_order_sn_list: id, page_number: 1, page_size: 50, sort_type: 1, po_sn_list: [id] })
    })
      .then((response) => response.json())
      .then((data) => {
        if (data.error_code == 40001 || !data?.result) {
  ...
  async function getOrderPackageShippingLabel(id) {
    return fetch('/mms/eagle/package/batch_print_shipping_label', {
      ...
      body: JSON.stringify({ package_sn_list: [id], merge_files: true })
    })
      .then((response) => response.json())
      .then((data) => {
        if (data.error_code == 40001 || !data?.result) {
        ...
        return (data?.result?.shipping_label_info_list ?? [])?.map((item) => {
          item.shipping_label_url = item.shipping_label_url || data?.result?.merged_shipping_label_url;
          return item;
        })?.[0];
      })
      .catch((error) => {
      ...
      orderList.forEach(({ productSkuIdList, orderSn, originalSpecName, originalGoodsName, goodsName, quantity = 1, orderStatus }) => {
        ...
        productSkuIdList?.forEach((sku) => {
          ...
          orders.push({
            ...
            orderId: orderSn,
            sku,
            title: currentDataMap[sku]?.title || `${originalSpecName ? `【${originalSpecName}】 ` : ''}${goodsName || originalGoodsName}`,
            quantity,
            price,
            costPrice,
            profit,
      ...
      for (var i = 0; i < packagesSource.length; i++) {
        try {
          if (packagesSource[i]?.package_sn) {
            const { package_sn, tracking_number, ship_company_name, ship_logistics_type, shipping_label_url } =
              (await getOrderPackageShippingLabel(packagesSource[i]?.package_sn)) ?? {};
            if (package_sn && tracking_number && shipping_label_url) {
              const dataSource = await fetch(shipping_label_url)
                .then((response) => {
                  if (!response.ok) {
  ...
  function orderUpdate() {
    ...
    orderEles
      .filter((ele) => ele?.childNodes?.[0]?.nodeValue?.match?.(/^PO\-[\d]+\-[\d]+/))
      .forEach((ele) => {
        ...
        currentOrderData
          .filter((item) => item.parentOrderId === parentOrderId)
          ?.forEach(({ id, title, price, costPrice, profit, profitMargin, quantity, tag, settled }) => {
            const span = document.createElement('span');
    ...
    document.body.addEventListener('click', async (e) => {
      ...
      if (sku && stockRemove) {
        ...
        if (productId && skuId && (keyMap['Control'] || confirm('是否清空商品库存'))) {
          ...
          if (list?.length) {
            try {
              await preCheckWarehouseInfo(
                productId,
                skuId,
                list.map(({ warehouseId }) => warehouseId)
              );
  ...
  function init() {
    const tableObserver = new MutationObserver(
      debounce((list) => {
        if (
          list.some((item) =>
            [...item.removedNodes, ...item.addedNodes].some(
              (ele) => !ele?.closest?.('.temu_plugin_sku_extra') && !ele?.closest?.('.temu_plugin_order_extra') && !ele?.closest?.('.temu_plugin_order_face_sheet')
            )
          )
        ) {
          update();
        ...
        const timer = setInterval(async () => {
          ...
          if (prevWarehouse && inputEle && !inputEle?.value) {
            ...
            const timer2 = setInterval(() => {
              const ele = findElementsByText(prevWarehouse)
                .find((e) => e.closest('li[role="option"]'))
  ...
  function orderInit() {
    ...
    const syncCheckout = async (silence, callback) => {
      if (silence || confirm(`同步近${checkoutDays > 1 ? checkoutDays : '今'}天已结算订单金额，是否继续？\n【注意】: 订单原价为 0 时将被设置为订单售价一致`)) {
        ...
        const getExportHistory = (merchantMerchantFileExportHistoryList) =>
          merchantMerchantFileExportHistoryList?.find((item) => item.searchExportTimeBegin <= beginTime && item.searchExportTimeEnd >= endTime);
    ...
    if (agentSeller) {
      ...
      if (window.location.host.includes('agentseller-us.temu.com') && !AutoDisabled) {
        autoSyncCheckout();
        setExactInterval(
          async () => {
            autoSyncCheckout();
    ...
    const pollingChat = async () => {
      ...
      const chats =
        res?.result?.data?.filter(({ message } = {}) => {
  ...
  const sellerInit = async () => {
    if (!AutoDisabled && window.location.host.includes('agentseller.temu.com')) {
      ...
      const priceUpdate = async () => {
        ...
        const items = !config.PriceAdjustNotice
          ? []
          : await fetch('/api/kiana/magnus/mms/price-adjust/page-query', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                mallid: await getMallId()
